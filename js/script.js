...
async function getZmones() {
    try {
        ...
        const dataJson = await fetch("https://rolandasseputis.lt:3343/json/zmones");   //console.log('datajson: ', dataJson);
        if (dataJson.ok) {
...
async function newRecord() {
    ...
    try {
        const dataJson = await fetch('https://rolandasseputis.lt:3343/json/zmones/', {
            method: "POST",
...
async function deleteRecord (event) {
    try {// nurodom id, kurį įrašą trinti
        const dataJson = await fetch('https://rolandasseputis.lt:3343/json/zmones/' + event.target.attributes.zmogusId.nodeValue,
        {method: 'DELETE'}
        );
        if (dataJson.ok) {
            getZmones();
